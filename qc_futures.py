...
# =============================================================================
# HARDCODED FEATURE LIST - Must match training exactly (90 features)
# =============================================================================
FEATURES = [
...
class XGBoostFuturesAlgorithm(QCAlgorithm):
    """
    Futures Trading Algorithm with XGBoost model loaded from API.
    ...
    def Initialize(self):
        ...
        self.model_key = "latest_model_futures.joblib"
        self.dataset_summary_key = "dataset_summary_futures.txt"
        ...
        # Offline model full feature list (90 features for futures)
        self.model_features = FEATURES

        # ===== Strategy meta =====
        self.strategy_name = "Metode: Futures v1 - DragonFortune"
        self.strategy_id = 5
        ...
        self.Debug("XGBoostFuturesAlgorithm initialized")
    ...
    def LoadDatasetSummaryFromServer(self):
        ...
        try:
            ...
            for fmt in dt_formats:
                try:
                    start_dt = datetime.strptime(start_raw.split()[0] if ' ' in start_raw and len(fmt.split()) == 1 else start_raw, fmt.split()[0] if ' ' in fmt else fmt)
                    break
                except Exception:
                    continue
            ...
            if end_raw:
                for fmt in dt_formats:
                    try:
                        end_dt = datetime.strptime(end_raw.split()[0] if ' ' in end_raw and len(fmt.split()) == 1 else end_raw, fmt.split()[0] if ' ' in fmt else fmt)
                        break
                    except Exception:
                        continue
