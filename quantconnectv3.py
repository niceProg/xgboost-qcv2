...
class XGBoostTradingAlgorithm(QCAlgorithm):
    ...
    def Initialize(self):
        ...
        try:
            ...
            self.Debug(f"Failed to load dataset summary from API: {e}")
            # Fallback dates
        ...
        # ===== Cash & Leverage Settings =====
        self.initial_cash = 100000          # Starting cash
        self.leverage = 10                  # Leverage multiplier (1x = no leverage, 10x = 10x leverage)
        self.SetCash(self.initial_cash)

        self.Debug(f"=== ACCOUNT SETTINGS ===")
        self.Debug(f"Initial Cash: ${self.initial_cash:,.0f}")
        self.Debug(f"Leverage: {self.leverage}x")
        self.Debug(f"========================")
        ...
        # Apply leverage to the crypto security
        try:
            self.Securities[self.symbol].SetLeverage(self.leverage)
            self.Debug(f"Leverage set to {self.leverage}x for {self.symbol}")
        except Exception as e:
            self.Debug(f"Warning: Could not set leverage: {e}")
        ...
        # Always try to load model from API
        try:
            self.Debug(f"Loading model from API: {self.api_base_url}/api/v1/futures/latest/model")
            self.LoadModelFromAPI()
        except Exception as e:
            self.Error(f"Failed to load model from API: {e}")
    ...
    def LoadDatasetSummaryFromAPI(self):
        """Load dataset summary from XGBoost API (futures endpoint) using QC Download()."""
        for attempt in range(self.api_retry_count):
            try:
                ...
                # Use QuantConnect's built-in Download method (can pass through firewall)
    ...
    def LoadModelFromAPI(self):
        """Load XGBoost model from API (futures endpoint) using QC Download()."""
        for attempt in range(self.api_retry_count):
            try:
