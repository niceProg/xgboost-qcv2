...
if [ ! -d "$MODEL_DIR" ]; then
    ...
    echo "1. You're already in the correct folder"
    echo "2. ./simple_run.sh (run historical training)"
    echo "3. You're already in the correct folder"
...
if [ -d "./output_train" ]; then
    ...
    if [ -d "./output_train/models" ] && [ -d "./output_train/datasets" ]; then
        ...
        print_warning "   python3 restructure_output_train.py"
    fi
else
...
RUN pip install --upgrade pip && \
    ...
    python3 -c "import pandas; pandas.options.display.max_rows=10; print('✅ Pandas can use PyArrow engine')" && \
...
RUN pip install --upgrade pip && \
    ...
    python3 -c "import sklearn; print('✅ Scikit-learn module available')" && \
    echo "✅ All critical dependencies verified"
...
# Skip core file verification to avoid import issues during build
# Core files will be verified at runtime instead
RUN echo "⚡ Skipping core file verification - will be done at runtime" && \
